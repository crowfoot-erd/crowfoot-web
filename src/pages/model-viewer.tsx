 ...
 * - 에디터 본체(캔버스·편집·content 저장)는 2단계 — 현재는 준비 중 안내까지만
 */
      ...
      {model.isPending ? (
        ...
        <>
          <header className="flex items-center justify-between gap-4 border-b px-4 py-3">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="shrink-0 text-xs text-muted-foreground">v{model.data.version}</span>
              </div>
              <p className="truncate text-xs text-muted-foreground">
                {[
                  `${model.data.canvasWidth}×${model.data.canvasHeight}`,
          ...
          <main className="relative flex-1 overflow-hidden">
            {/* 에디터 캔버스 영역 — 2단계에서 편집 화면으로 교체한다 */}
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                <p className="font-medium">{t('model.viewer.pending.title')}</p>
                <p className="mt-1 text-sm text-muted-foreground">
                  {t('model.viewer.pending.description')}
                </p>
              </div>
            </div>
          </main>
