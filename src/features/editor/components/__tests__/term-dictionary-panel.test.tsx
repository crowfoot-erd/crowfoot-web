/**
 * 용어 사전 패널 테스트 — 두 탭(표준·시스템)·등록·수정 다이얼로그(타입 문서 DB 1칸)·
 * 시스템탭 서버 페이징·알파벳 이니셜·열람 전용·권한 게이트 (용어 사전 패널, 05-editor/02-ui.md)
 *
 * 표준 사전은 MSW 목업(fixtures.terms: member→회원, user→사용자 types {mysql:
 * VARCHAR(60), postgresql: VARCHAR(50)}), 시스템 사전은 fixtures.systemTerms 5건(email·id·
 * user·yn·zipcode — types는 DBMS별 맵, '#' 이니셜 없음). 문서의 DB 종류는 postgresql —
 * 타입 접미·다이얼로그 저장 키의 기준. 목업은 픽스처 불변 원칙이라 등록·삭제 후 목록이
 * 원본으로 돌아온다 — 요청 반영은 스파이로, 화면 반영은 다이얼로그 프리필·닫힘으로 검증한다.
 */
...
describe('TermDictionaryPanel — 표준 사전 탭', () => {
  ...
  it('Viewer(canEdit=false)는 목록·검색만 되고 등록 버튼·삭제가 없다', async () => {
